...
export async function sendWelcomeEmail({ email, name, type }: { email: string; name: string; type: 'volunteer' | 'member' }) {
  const isVolunteer = type === 'volunteer';
  const subject = isVolunteer ? 'Welcome to the Volunteer Team!' : 'Welcome to the Sewarth Path Family!';
  const greeting = isVolunteer ? 'Welcome to our volunteer network!' : 'Welcome as a member of our community!';
  const nextSteps = isVolunteer 
    ? 'Our Admin will review your application and contact you via WhatsApp soon to discuss the next steps.' 
    : 'Thank you for becoming a member. You can now access member-exclusive resources and updates.';
  ...
  return resend.emails.send({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px;">
        ...
        <p>${greeting}</p>
        <p>${nextSteps}</p>
        <p>We are thrilled to have you with us in our journey of selfless service.</p>
        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; color: #888; font-size: 0.8em;">
          <p>&copy; ${new Date().getFullYear()} Sewarth Path Sansthanam. All rights reserved.</p>
        </div>
      </div>
